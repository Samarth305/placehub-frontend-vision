...
export const Route = createFileRoute("/login")({
  head: () => ({
    meta: [
      { title: "Sign in — PlaceHub" },
      { name: "description", content: "Sign in to PlaceHub." },
    ],
  }),
  component: LoginPage,
});
...
function LoginPage() {
  ...
  const handleLogin = async () => {
    try {
      ...
      (typeof window !== 'undefined' ? localStorage.setItem("token", res.accessToken) : null);
      (typeof window !== 'undefined' ? localStorage.setItem("refreshToken", res.refreshToken) : null); // <-- Save the refresh token!
    ...
    <AuthShell
      ...
      footer={
        <>
          ...
          <Link to="/signup" className="text-primary hover:text-primary-glow">
            Create one
          </Link>
        </>
      }
